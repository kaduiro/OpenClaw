...
Control UI の prompt では、これらを workspace root 相対で指定します。
...
```text
docs/projects/<project-name>.md と、inbox/raw/ 内の関連メモを参照してください。
...
このリポジトリを AI 秘書として使う場合、事前調査は単発で終わらせません。
次のループで回します。

1. `workspace/docs/projects/<project-name>.md` に brief を残す
2. Control UI から保存先付きで要約や事前調査を依頼する
3. 再利用する成果物は `workspace/docs/projects/` に残す
4. 断片メモは `workspace/inbox/raw/` に落とす
5. nightly triage で memory と next actions に整理する
6. 翌朝、morning brief を見て再着手する

現状の unattended job で自動化されるのは、メモ整理と朝の要約までです。
技術調査そのものは Control UI から依頼し、その結果を Markdown として蓄積する運用で成立させます。
